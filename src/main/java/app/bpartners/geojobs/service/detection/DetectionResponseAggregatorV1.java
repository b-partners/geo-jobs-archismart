...
@Slf4j
@Component
public class DetectionResponseAggregatorV1
    implements Function<
        List<DetectionResponseAggregatorV1.DetectionResponseUrl>, DetectionResponse> {
  ...
  public DetectionResponse apply(List<DetectionResponseUrl> responseUrls) {
    ...
    for (var r : responseUrls) {
      ...
      if (response.getRstRaw() != null) {
        for (Map.Entry<String, DetectionResponse.ImageData> entry :
            response.getRstRaw().entrySet()) {
      ...
      if (response.getVegetationData() != null
          && response.getVegetationData().getRegions() != null
